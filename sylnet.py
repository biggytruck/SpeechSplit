...
modelFile = mainFile + '/sylnet_models/'
means_path = modelFile + 'means.npy'
std_path = modelFile + 'stds.npy'
model_path = modelFile + 'model_trained.ckpt'


def get_speaking_rate(fname_dir):

    ########### GET DATA #############
    wasdir = 0
    # If input
    if(os.path.isdir(fname_dir)):
        fileList = sorted(glob.glob(os.path.join(fname_dir, '*.wav')))
        wasdir = 1
        if(len(fileList) == 0):
            raise Exception('Provided directory contains no .wav files')
    elif(fname_dir.endswith('.wav')):
        fileList = list()
        fileList.append(fname_dir)
    else:
        if(os.path.isfile(fname_dir)):
            fileList = list(filter(bool,[line.rstrip('\n') for line in open(fname_dir)]))
        else:
    ...
    result_dict = {}
    for i, (y, fname) in enumerate(zip(Y, fileList)):
        basename = os.path.basename(fname)
        sound = AudioSegment.from_wav(fname)
        start_trim = silence.detect_leading_silence(sound, silence_threshold=-25.0, chunk_size=5)
        end_trim = silence.detect_leading_silence(sound.reverse(), silence_threshold=-25.0, chunk_size=5)
        duration = len(sound)    
